...
A prior exploratory study with a more complex repeated-measures design revealed a small effect size for the moderating role of a prevention focus on risk-taking within a hypothetical investment allocation task, with an $f^2$ of 0.021. A power analysis based on this effect size suggested a required sample size of 462 to detect the interaction of interest with sufficient power; to account for expected attrition between Parts I and II of the present study, we enrolled 500 participants.

Data collection took place in September 2018, and our full sample was recruited through an advertisement on Amazon's Mechanical Turk ("M-Turk") platform soliciting participation from workers at least 18 years of age who were based in the United States. 
...
# Rerunning with Participants Who Learned About Study from Other M-Turkers
...
```{r desc.rerun, echo = FALSE, warning = FALSE, message = FALSE}
# summarySE() function defined at top of document
...
When rerunning our simple slopes and Johnson-Neyman analyses, the significance and direction of our results do not change. The effect of prevention pride on Bitcoin allocation was still significant in the counterfactual loss condition ($B = `r myround(jn1.rerun[1,"estimate"], digits = 2)`, SE = `r myround(jn1.rerun[1,"std.error"], digits = 2)`, t = `r myround(jn1.rerun[1,"statistic"], digits = 2)`, `r pval(as.numeric(jn1.rerun[1,"p.value"]))`$; 95% CI [`r myround(jn1.rerun[1,"conf.low"], digits = 4)`, `r myround(jn1.rerun[1,"conf.high"], digits = 4)`]) but non-significant in the control condition.

Further, the conditional effect of counterfactual loss on Bitcoin allocation was still positive for people with prevention pride of less than 2.51 ($B = `r myround(jn2.rerun[2,"estimate"], digits = 2)`, SE = `r myround(jn2.rerun[2,"std.error"], digits = 2)`, t = `r myround(jn2.rerun[2,"statistic"], digits = 2)`, `r pval(as.numeric(jn2.rerun[2,"p.value"]))`$), and negative for people with prevention pride of greater than 4.57 ($B = `r myround(jn2.rerun[1,"estimate"], digits = 2)`, SE = `r myround(jn2.rerun[1,"std.error"], digits = 2)`, t = `r myround(jn2.rerun[1,"statistic"], digits = 2)`, `r pval(as.numeric(jn2.rerun[1,"p.value"]))`$).
