...
When rerunning our simple slopes analysis, the direction and significance of our results do not change. The effect of prevention pride on Bitcoin allocation was still significant in the counterfactual loss condition ($B = `r myround(jn1.rerun[2,"estimate"], digits = 2)`, SE = `r myround(jn1.rerun[2,"std.error"], digits = 2)`, t = `r myround(jn1.rerun[2,"statistic"], digits = 2)`, `r pval(as.numeric(jn1.rerun[2,"p.value"]))`$; 95% CI [`r myround(jn1.rerun[2,"conf.low"], digits = 4)`, `r myround(jn1.rerun[2,"conf.high"], digits = 4)`]) but non-significant in the control condition.
...
The direction and significance of our primary linear analysis does not change when rerunning the analysis to include every single participant who completed our independent and dependent measures, even those whose data had questionable validity due to IP spoofing issues as described above (see Table 15 and Figure 3).

```{r subset4.all, include=FALSE}
# Creating subset to facilitate visualization of results within each condition
...
```{r primarymodel.all, include = FALSE}
# PRIMARY MODEL, CONTROLLING FOR PROMOTION (mean-centered RF and dummy-coded condition)
rfcfl.promctr.all <- lm(allo ~ premc * condD + promc * condD, data=cfl.all)
summary(rfcfl.promctr.all) # premc:condD b = -7.98, p = .046
```

```{r primarymodelalltable, include = FALSE}
...
```{r primarymodelallprint, echo = FALSE, include = TRUE, results = 'asis'}
apa_table(table1a, caption = "RERUN (ALL): Summary of Linear Regression Analysis", note = "This analysis included a dummy-coded variable for the counterfactual loss manipulation: 0 = control, 1 = counterfactual loss. Additionally, prevention and promotion pride were mean-centered within this model. Estimated effect sizes reported here are unstandardized regression coefficients.", align = 'lrrrr')
```

```{r primarymodelplot.all, echo = FALSE}
...
primarymodel.all <- ggplot(data=cfl.all, aes(x=premc, y=allo)) +
       scale_color_manual(values=c("gray0", "gray50")) +
       geom_ribbon(data = rfcfl.promctr.pred.ctrl, aes(ymin = lwr, ymax = upr), alpha = .3, fill = "gray") +
       geom_ribbon(data = rfcfl.promctr.pred.CFL, aes(ymin = lwr, ymax = upr), alpha = .3, fill = "gray") +
       geom_line(data = rfcfl.promctr.pred.ctrl, aes(color=condition), size = 1) +
       geom_line(data = rfcfl.promctr.pred.CFL, aes(color=condition), size = 1) +
       labs(title="RERUN (ALL): Prevention Pride and Counterfactual Loss\nas Predictors of Bitcoin Allocation\n(Controlling for Promotion Pride and the Interaction\nBetween Promotion Pride and Counterfactual Loss)", x="Prevention Pride (Mean-Centered)", y="Bitcoin Allocation (%)", color = "Condition")
```

```{r primarymodelplot_print.all, echo = FALSE, include = TRUE, fig.cap = "RERUN (ALL): Predicted Bitcoin allocation by prevention pride and counterfactual loss experience, controlling for promotion pride and the interaction between promotion pride and counterfactual loss.", fig.align = "center", out.width = '90%', out.height = '90%'}
primarymodel.all
```

## Simple Slopes Analysis

```{r jn1_simpleslopes.all, echo = FALSE}
...
When rerunning our simple slopes analysis to include all participants who completed our independent and dependent measures, the direction and significance of our results do not change. The effect of prevention pride on Bitcoin allocation was still significant in the counterfactual loss condition ($B = `r myround(jn1.all[2,"estimate"], digits = 2)`, SE = `r myround(jn1.all[2,"std.error"], digits = 2)`, t = `r myround(jn1.all[2,"statistic"], digits = 2)`, `r pval(as.numeric(jn1.all[2,"p.value"]))`$; 95% CI [`r myround(jn1.all[2,"conf.low"], digits = 4)`, `r myround(jn1.all[2,"conf.high"], digits = 4)`]) but non-significant in the control condition.

## Exploratory Mediation Analysis (Hayes PROCESS Model 8): Counterfactual Relief

```{r med.relieved.promctr.lavaan.all, eval = FALSE, include = FALSE, warning = FALSE, message = FALSE, cache = TRUE}
# Fit model (no changes in model from original specification)
set.seed(1234)
...
```{r med.relieved.promctr.lavaan.vis.all, echo = FALSE, include = FALSE, warning = FALSE, message = FALSE, cache = FALSE}
...
```{r medmodel.relieved.all, echo = FALSE, include = FALSE}
...
```{r medmodel.relieved.all.print, echo = FALSE}
medmodel.relieved.all
```

When we control for promotion pride and the interaction between promotion and counterfactual loss, the interaction between prevention and counterfactual loss was still significantly associated with the degree to which participants reported they would be relieved if they had invested in Bitcoin earlier ($\beta = `r myround(a3.relieved.promctr.all, digits = 2)`$, $`r pval(as.numeric(a3.p.relieved.promctr.all))`$), and the effect of participants reporting they would be relieved if they had invested earlier ($\beta = `r myround(b.relieved.promctr.all, digits = 2)`$, $`r pval(as.numeric(b.p.relieved.promctr.all))`$) on Bitcoin allocation was also significant.

The indirect effect of prevention on Bitcoin allocation through reporting relief if they had invested earlier was significant and negative for individuals in the counterfactual loss condition ($\beta = `r myround(condIndEffCFL.relieved.promctr.all, digits = 2)`$, $SE = `r myround(condIndEffCFL.se.relieved.promctr.all, digits = 2)`$, 95% $CI = `r myround(condIndEffCFL.cilower.relieved.promctr.all, digits = 4)`, `r myround(condIndEffCFL.ciupper.relieved.promctr.all, digits = 4)`$), but not for individuals in the control condition (n.s.). This indicates that the moderation is still fully mediated.

\newpage

# Summary Table

```{r summarytable, echo = FALSE, include = TRUE}
summarytable1 <- dplyr::filter(table1, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1$Predictor <- recode(summarytable1$Predictor, "Prevention Pride x Counterfactual Loss" = "Original Model")

summarytable1.gender <- dplyr::filter(table1.gender, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1.gender$Predictor <- recode(summarytable1.gender$Predictor, "Prevention Pride x Counterfactual Loss" = "Controlling for Gender")

summarytable1.age <- dplyr::filter(table1.age, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1.age$Predictor <- recode(summarytable1.age$Predictor, "Prevention Pride x Counterfactual Loss" = "Controlling for Age")

summarytable1.ethnicity <- dplyr::filter(table1.ethnicity, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1.ethnicity$Predictor <- recode(summarytable1.ethnicity$Predictor, "Prevention Pride x Counterfactual Loss" = "Controlling for Ethnicity")

summarytable1.income <- dplyr::filter(table1.income, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1.income$Predictor <- recode(summarytable1.income$Predictor, "Prevention Pride x Counterfactual Loss" = "Controlling for Income")

summarytable1.edu <- dplyr::filter(table1.edu, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1.edu$Predictor <- recode(summarytable1.edu$Predictor, "Prevention Pride x Counterfactual Loss" = "Controlling for Education")

summarytable1.promctrnointxn <- dplyr::filter(table1.promctrnointxn, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1.promctrnointxn$Predictor <- recode(summarytable1.promctrnointxn$Predictor, "Prevention Pride x Counterfactual Loss" = "Not Controlling for Prom. Pride x CF Loss Interaction")

summarytable1.noprom <- dplyr::filter(table1.noprom, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1.noprom$Predictor <- recode(summarytable1.noprom$Predictor, "Prevention Pride x Counterfactual Loss" = "Not Controlling for Prom. Pride or Prom. Pride x CF Loss Interaction")

summarytable1r <- dplyr::filter(table1r, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1r$Predictor <- recode(summarytable1r$Predictor, "Prevention Pride x Counterfactual Loss" = "Rerun with Ps Who Learned About Study from Others")

summarytable1a <- dplyr::filter(table1a, Predictor == "Prevention Pride x Counterfactual Loss")
summarytable1a$Predictor <- recode(summarytable1a$Predictor, "Prevention Pride x Counterfactual Loss" = "Rerun with All Ps Who Completed IV and DV")

summarytable <- rbind(summarytable1, summarytable1.gender, summarytable1.age, summarytable1.ethnicity, summarytable1.income, summarytable1.edu, summarytable1.promctrnointxn, summarytable1.noprom, summarytable1r, summarytable1a)
summarytable <- dplyr::rename(summarytable, Model = Predictor)
summarytable$SE <- myround(as.numeric(summarytable$SE), digits = 2)
summarytable$t <- myround(as.numeric(summarytable$t), digits = 2)

summarytable <- summarytable %>%
  mutate(
    Model = cell_spec(Model, "latex", align = "l"),
    Estimate = cell_spec(Estimate, "latex", align = "r"),
    SE = cell_spec(SE, "latex", align = "r"),
    t = cell_spec(t, "latex", align = "r"),
    p = cell_spec(p, "latex", align = "r")
  ) %>%
  kable("latex", booktabs = TRUE, escape = FALSE, linesep = "", caption = "Effect of Interest (Prevention Pride x Counterfactual Loss Interaction) Across All Linear Regression Analyses") %>%
  kable_styling(latex_options = "HOLD_position") %>%
  footnote(general = "This analysis included a dummy-coded variable for the counterfactual loss manipulation: 0 = control, 1 = counterfactual loss. Estimated effect sizes reported here are unstandardized regression coefficients.", general_title = "Note: ", footnote_as_chunk = T, title_format = c("italic"), threeparttable = TRUE, escape = FALSE)
```

```{r summarytableprint, echo = FALSE}
summarytable
```
